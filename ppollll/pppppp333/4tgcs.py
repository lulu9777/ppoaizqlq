...
bot_token = "66xxxxxx40:AAxxxxxxxxnp_1UjkxxxxxxxxxJEk"
chat_ids = ["-10xxxxxxxx96", "-10xxxxxxxx36", "-100xxxxxxxxx12", "17xxxxxxxx04"]
file_path_predictions = os.path.join(os.getcwd(), "./predictions.txt")
last_request_time = 0

def send_message(update, context):
    global last_request_time
    if update.effective_message.text.lower() == "lq" and str(update.effective_chat.id) in chat_ids:
        ...
        if "此时没有第3节的比赛" in result.stdout:
            context.bot.send_message(chat_id=update.effective_chat.id, text="此时没有第3节的比赛")
            return
