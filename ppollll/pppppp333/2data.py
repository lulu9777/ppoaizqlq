...
# 只保留match_duration列为'第3节'或'中'的数据
mid_game_data = data[data['match_duration'].isin(['第3节', '中场'])].copy()
...
# 计算半场总分单双
data.loc[:, 'half_score_parity'] = (data['half_score']) % 2
...
# 归一化半场得分
scaler = MinMaxScaler()
scaled_half_scores = scaler.fit_transform(data[['half_score']]).astype(np.float32)
...
df = pd.DataFrame(processed_data, columns=['id', 'league_type', 'team1', 'team2', 'half_score', 'match_time', 'half_score_parity'])
